...
this.main=this.ha.shadowRoot.querySelector("home-assistant-main").shadowRoot,this.user=this.ha.__hass.user,this.llAttempts=0,this.run(),
...
var e=t.__lovelace.config.kiosk_mode||{};this.processConfig(t,e)}catch(e){this.llAttempts<200?setTimeout(function(){return r.getConfig(t)},50):(
...
value:function(e,t){var r=this,n=this.ha.__hass.panelUrl;window.kioskModeEntities[n]||(window.kioskModeEntities[n]=[]),
...
;window.kioskModeEntities[n].includes(d)||window.kioskModeEntities[n].push(d),this.ha.__hass.states[d].state==c.entity[d]&&("hide_header"in c&&(
...
"ha-drawer"),e=t.querySelector(".toolbar");this.hideHeader||this.hideOverflow?(this.addStyle("".concat(
this.hideHeader?"#view{min-height:100vh !important;--header-height:0;}.header{display:none;}":"").concat(
...
":host{--mdc-drawer-width:0 !important;}ha-sidebar{display:none;}",r),this.addStyle("ha-menu-button{display:none !important;}",e),this.queryString(
...
var t=window.kioskModeEntities[this.ha.__hass.panelUrl]||[];!t.length||"state_changed"!=e.event_type||!t.includes(e.data.entity_id
...
)&&e.querySelector("#kiosk_mode_".concat(e.localName)).remove()})}}]),e}(),d={header:"%câ‰¡ kiosk-mode".padEnd(27),ver:"%cversion 1.7.2 "},
...
m[y],1)[0];d[v].length<=p&&(d[v]=d[v].padEnd(p)),"header"==v&&(d[v]="".concat(d[v].slice(0,-1),"â‹® "))}
